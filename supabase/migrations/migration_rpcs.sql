-- ============================================
-- RPCs: Grupos e Mensagens Fixadas
-- ============================================
...
BEGIN
  ...
  -- Adicionar o dono como participante com role 'owner'
  INSERT INTO public.chat_participants (chat_id, user_id, role)
...
BEGIN
  ...
  -- Verificar que é um grupo e que o usuário atual é o dono
...
BEGIN
  ...
  -- Verificar que é um grupo e que o usuário é o dono
  ...
  -- Validar duração (24h ou 168h = 7 dias)
  ...
  -- Inserir ou atualizar mensagem fixada
...
BEGIN
  ...
  -- Inserir read receipts para mensagens que eu ainda não li (que não são minhas)
...
-- Permitir UPDATE no status de messages (para mark as delivered)
GRANT UPDATE ON public.messages TO authenticated;

-- Policy de UPDATE em messages (só pode atualizar status)
